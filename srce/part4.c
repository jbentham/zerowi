...
//
// v0.61 JPB 22/3/20  Tidied up for publishing on github
...
char event_msgs[EVENT_MAX / 8];
uint8_t eventbuff[1024];
...
int main(void)
{
    ...
    n = ioctl_getvar("cur_etheraddr", eth, 6);
    printf("MAC address ");
    if (n)
        disp_mac_addr(eth);
    else
        printf("unavailable");
    n = ioctl_getvar("ver", resp, sizeof(resp));
    printf("\nFirmware %s\n", (n ? (char *)resp : "not responding"));
    ioctl_cmd_int32(IOCTL_SET_SCAN_CHANNEL_TIME, SCAN_CHAN_TIME);
    ioctl_cmd_int32(IOCTL_UP, 0);
    n = 0;
    while (sdio_bak_read32(SB_INT_STATUS_REG, &val) && !(val&0xff))
    {
        usdelay(50000);
        if (n++ == 10)
        {
